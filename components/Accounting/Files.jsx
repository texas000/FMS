export default function Files({ files }) {
  async function handleDownload(file, ref, available) {
    window.location.assign(
      `/api/file/get?ref=${ref}&file=${encodeURIComponent(file)}`
    );
  }
  async function handleOpen(file, ref) {
    const data = await fetch(
      `/api/file/get?ref=${ref}&file=${encodeURIComponent(file)}`
    );
    const blob = await data.blob();
    var download = new Blob([blob], {
      type: blob.type,
    });
    var fileURL = URL.createObjectURL(download);
    window.open(fileURL);
  }
      ...
      return (
        <div className="card col-span-2 rounded-xl mt-3 text-gray-800 overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="dark:text-white divide-y divide-gray-200 px-2 py-3 text-left text-xs font-medium uppercase tracking-wider">
              {files?.file?.map((ga, i) => (
                <tr key={`${i}-file`}>
                  <td
                    className={`px-6 py-2 whitespace-nowrap w-1/2 cursor-pointer ${
                      ga.F_SECURITY
                        ? "hover:bg-indigo-500 hover:text-white text-gray-500"
                        : "text-gray-200"
                    }`}
                    onClick={() =>
                      handleDownload(ga.F_FILENAME, ga.F_REF, ga.F_SECURITY)
                    }
                  >
                    {ga.F_LABEL} - {ga.F_FILENAME}
                  </td>
                  <td
                    className={`px-6 py-2 whitespace-nowrap w-1/4 cursor-pointer text-center ${
                      ga.F_SECURITY
                        ? "hover:bg-indigo-500 hover:text-white text-gray-500"
                        : "text-gray-200"
                    }`}
                    onClick={() => handleOpen(ga.F_FILENAME, ga.F_REF)}
                  >
                    <svg
                      width="18"
                      height="18"
                  ...
                  <td
                    className="px-6 py-2 text-center whitespace-nowrap hover:bg-indigo-500 hover:text-white uppercase w-1/4 cursor-pointer text-gray-500"
                    onClick={() => handleHide(ga.F_ID)}
                  >
                    delete
                  </td>
                </tr>
