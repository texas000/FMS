...
const MAIN = ({ DATA, ADD }) => {
	...
	const mailSubject = `[JW] ${MASTER.ACCOUNT} MBL# ${MASTER.MBL} HBL# ${
		MASTER.HBL
	} CNTR# ${
		DATA.CONTAINER && DATA.CONTAINER.map((ga) => `${ga.F_ContainerNo}`)
	}ETD ${moment(MASTER.ETD).add(1, "days").format("l")} ETA ${moment(MASTER.ETA)
		.add(1, "days")
		.format("l")} // ${MASTER.REF}`;
	const mailBody = `Dear ${MASTER.ACCOUNT}
    \nPlease note that there is an OCEAN SHIPMENT for ${
			MASTER.ACCOUNT
		} scheduled to depart on ${moment(MASTER.ETA)
		.add(1, "days")
		.format("LL")}.\n${EmailText && `\n${EmailText}`}
	...
	var emailHref = `mailto:${mailTo}?cc=${mailCC}&subject=${encodeURIComponent(
		mailSubject
	)}&body=${encodeURIComponent(mailBody)}`;

	const mailSubjectWH = `[JW] WAREHOUSE ASN FOR ${MASTER.ACCOUNT} / CNTR# ${
		DATA.CONTAINER && DATA.CONTAINER.map((ga) => `${ga.F_ContainerNo} `)
	} ETA ${moment(MASTER.ETA).add(1, "days").format("l")} // ${MASTER.REF}`;
	...
	var emailHrefWH = `mailto:${WHmailTO}?cc=${encodeURIComponent(
		WHmailCC
	)}&subject=${encodeURIComponent(mailSubjectWH)}&body=${encodeURIComponent(
		mailBodyWH
	)}`;
