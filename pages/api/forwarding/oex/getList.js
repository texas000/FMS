...
export default async (req, res) => {
	...
	if (req.headers.search) {
		...
		Query = `SELECT M.F_ID, M.F_RefNo, (SELECT TOP 1 (SELECT F_SName from T_COMPANY C where C.F_ID=H.F_Customer) FROM T_OOHMAIN H WHERE H.F_OOMBLID=M.F_ID) as Customer FROM T_OOMMAIN M WHERE M.F_RefNo like '%${q}%' ORDER BY F_ID DESC;`;
	}
