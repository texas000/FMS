...
export default async (req, res) => {
	...
	if (req.headers.search) {
		...
		Query = `SELECT M.F_ID, M.F_RefNo, (SELECT TOP 1 (SELECT F_SName from T_COMPANY C where C.F_ID=H.F_Customer) FROM T_AIHMAIN H WHERE H.F_AIMBLID=M.F_ID) as Customer FROM T_AIMMAIN M WHERE M.F_RefNo like '%${q}%' ORDER BY F_ID DESC;`;
	}
