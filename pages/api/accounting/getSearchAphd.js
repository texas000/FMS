  ...
  if (body) {
    switch (body.type) {
      ...
      case "inv":
        ...
        break;
      case "pic":
        qry = `select top 1000 *, (select F_SName from T_COMPANY where F_PayTo=T_COMPANY.F_ID) as VENDOR from T_APHD where F_U1ID like '%${body.value.replace(
          /'/g,
          ""
        )}%' OR F_U2ID like '%${body.value.replace(
          /'/g,
          ""
        )}%' order by F_ID desc;`;
        break;
