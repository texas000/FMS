...
const Index = ({ token }) => {
  ...
  const { data, mutate } = useSWR(
    `/api/forwarding/oex/pagination?page=${Page}&size=50`
  );
  ...
  function indication() {
    return (
      <span>
        <span className="text-danger">No Ocean Export</span> at the moment
      </span>
    );
  }
  ...
  const rowEvents = {
    onClick: (e, row, rowIndex) => {
      setLoading(true);
      router.push(`/forwarding/oex/${row.F_RefNo}`);
    },
  };

  return (
    <Layout TOKEN={token} TITLE="OCEAN EXPORT" LOADING={!data || loading}>
      <div className="d-flex flex-sm-row justify-content-between">
        <div className="flex-column">
          <h3 className="dark:text-white">Ocean Export</h3>
        </div>
      </div>
