...
const Index = ({ token }) => {
  ...
  const { data, mutate } = useSWR(
    `/api/forwarding/aim/pagination?page=${Page}&size=50`
  );
  ...
  const rowEvents = {
    onClick: (e, row, rowIndex) => {
      setLoading(true);
      router.push(`/forwarding/aim/${row.F_RefNo}`);
    },
  };

  return (
    <Layout TOKEN={token} TITLE="AIR IMPORT" LOADING={!data || loading}>
      <div className="d-flex flex-sm-row justify-content-between">
        <div className="flex-column">
          <h3 className="dark:text-white">Air Import</h3>
        </div>
      </div>
