...
const Detail = ({ token, Reference }) => {
  ...
  const { data: vendorFiles } = useSWR(
    selectedVendor ? "/api/file/list?ref=COMPANY-" + selectedVendor : null
  );
                    ...
                    <div className="m-2 shadow overflow-auto border-b border-gray-200 sm:rounded-lg">
                      <table className="min-w-full divide-y divide-gray-200">
                        <tbody className="bg-white dark:bg-gray-500 divide-y divide-gray-200 px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">
                          ...
                          <tr className="hover:bg-indigo-500 hover:text-white">
                            <td className="px-6 py-2 whitespace-nowrap">
                              Vendor
                            </td>
                            <td className="px-6 py-2 whitespace-nowrap">
                              <Popover2
                                      ...
                                      vendorFiles.map((ga) => (
                                        ...
                                        >
                                          <svg
                                            ...
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                            className="inline mr-1 hover:text-indigo-500 cursor-pointer"
                                            onClick={async () => {
                                              const data = await fetch(
                                                `/api/file/get?ref=COMPANY-${selectedVendor}&file=${encodeURIComponent(
                                                  ga.F_FILENAME
                                                )}`
                                              );
                                ...
                                minimal={true}
                                onOpening={() =>
                                  setSelectedVendor(selectedPayment.F_PayTo)
                                }
