...
const Index = ({Cookie, Board}) => {
  ...
  //  }
   const addNew = async () => {
    if(TOKEN.uid) {
      const value = (`N'${decodeURIComponent(title.replace("'", ""))}', N'${decodeURIComponent((body.replace('%0A', `'+CHAR(13)+N'`)))}', GETDATE(), 1, ${TOKEN.uid}`)
      console.log(value)
      const Fetch = await fetch("/api/jwboard/addPost", {body: value, method: "POST"})
       ...
       <>
         ...
         <Row>
             <Col>
               ...
               <p>JAMES WORLDWIDE 게시판</p>
               </Col>
               <div style={{ position: "fixed", right: "2rem", top: "2rem", color: '#4582ec' }} onClick={() => setModal(true)}> 
               <span>글쓰기</span>
               <i
...
export async function getServerSideProps({req}) {
    ...
    const Fetch = await fetch(`${process.env.BASE_URL}api/jwboard/getPost`)
