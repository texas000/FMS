...
export default function search(props) {
	...
	const [fileAccount, setFileAccount] = useState(false);
	const [filePasswd, setFilePasswd] = useState(false);
	...
	function handleSubmit() {
		if (newPassword || fileAccount || filePasswd) {
			fetch("/api/login/update", {
				method: "POST",
				body: JSON.stringify({
					pass: newPassword,
					facc: fileAccount,
				...
				if (ga.status === 200) {
					...
					alert(`ERROR ${ga.status}`);
				}
			});
		} else {
			alert("Please fill out at least a field");
		}
		// console.log(newPassword);
		// console.log(fileAccount);
	...
	return (
		<Layout TOKEN={props.token} TITLE="Profile">
			...
			<div className="grid lg:grid-cols-2 gap-4">
				<div className="card items-center p-3 gap-2">
					<img
					...
					<p className="flex w-100 justify-between p-2 rounded bg-gray-100 dark:bg-gray-600 uppercase">
					...
					<p className="flex w-100 justify-between p-2 rounded bg-gray-100 dark:bg-gray-600 uppercase">
						...
						<input
							...
							pattern="[a-zA-Z0-9]*"
							className="border-2 focus:ring-indigo-500 focus:border-indigo-500 block px-2 sm:text-sm border-gray-300 rounded-md"
						></input>
					</p>
					<p className="flex w-100 justify-between p-2 rounded bg-gray-100 dark:bg-gray-600 uppercase">
					...
					<p className="flex w-100 justify-between p-2 rounded bg-gray-100 dark:bg-gray-600 uppercase">
						<span className="font-bold">FILE SERVER ID</span>
						<input
							onChange={(e) => setFileAccount(e.target.value)}
							className="border-2 focus:ring-indigo-500 focus:border-indigo-500 block px-2 sm:text-sm border-gray-300 rounded-md"
						></input>
					</p>
					<p className="flex w-100 justify-between p-2 rounded bg-gray-100 dark:bg-gray-600 uppercase">
						<span className="font-bold">FILE SERVER PASS</span>
						<input
							onChange={(e) => setFilePasswd(e.target.value)}
							className="border-2 focus:ring-indigo-500 focus:border-indigo-500 block px-2 sm:text-sm border-gray-300 rounded-md"
						></input>
					</p>
					<button
